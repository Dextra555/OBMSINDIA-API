-- ============================================================
-- Add RBI Bank Advance Salary Process Screen to ScreenList
...
PRINT '=== Adding RBI Bank Advance Salary Process Screen ==='

-- Check if screen already exists
IF NOT EXISTS (SELECT 1 FROM ScreenList WHERE ScreenName = 'RBI Bank Advance Salary Process')
BEGIN
    PRINT 'Adding RBI Bank Advance Salary Process to ScreenList...'
    
    INSERT INTO ScreenList (ScreenName, Category, LastUpdate)
    VALUES ('RBI Bank Advance Salary Process', 'Payroll', GETDATE())
...
IF NOT EXISTS (
    ...
    WHERE Name = 'superadmin' AND ScreenName = 'RBI Bank Advance Salary Process'
)
BEGIN
    ...
    VALUES ('superadmin', 'RBI Bank Advance Salary Process', 1, 1, 1, 1, 'system')
...
INSERT INTO OBMSPermissions (Name, ScreenName, [Read], [Create], [Update], [Delete], LastUpdatedBy)
SELECT DISTINCT u.Name, 'RBI Bank Advance Salary Process', 1, 1, 1, 1, 'system'
FROM OBMSUsers u
WHERE NOT EXISTS (
    SELECT 1 FROM OBMSPermissions p 
    WHERE p.Name = u.Name AND p.ScreenName = 'RBI Bank Advance Salary Process'
)
...
PRINT ''
PRINT '=== Current Permissions for RBI Bank Advance Salary Process ==='
SELECT Name, ScreenName, [Read], [Create], [Update], [Delete], LastUpdatedBy
FROM OBMSPermissions
WHERE ScreenName = 'RBI Bank Advance Salary Process'
