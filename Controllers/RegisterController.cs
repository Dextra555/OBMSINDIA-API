...
namespace OBMS.WebAPI.Controllers
{
    ...
    public class RegisterController : Controller
    {
        ...
        [HttpPost]
        [Route("AddScreen")]
        public async Task<ActionResult<bool>> AddScreen(string screenName, string category)
        {
            try
            {
                var result = await _registerRepository.AddScreen(screenName, category);
                return Ok(result);
            }
            catch (Exception ex)
            {
